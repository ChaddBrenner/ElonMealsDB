...
describe('security and validation middleware', () => {
  ...
  it('rejects invalid nutrition insight dates before query execution', async () => {
    const response = await request(app).get('/api/metrics/nutrition-insights?date=2026-02-31');
    expect(response.status).toBe(400);
    expect(response.body.error.code).toBe('bad_request');
  });
